...
const QuestionCard: React.FC<QuestionCardProps> = ({
  ...
  return (
    <div className="bg-white/90 backdrop-blur-sm rounded-3xl shadow-2xl border border-purple-100 p-6 max-w-6xl mx-auto overflow-hidden">
      {/* Header with gradient background */}
      <div className="bg-gradient-to-r from-purple-500 via-pink-500 to-blue-500 -m-6 mb-4 p-4 text-white">
        <div className="flex justify-between items-center">
          <div className="flex items-center space-x-3">
            <div className="bg-white/20 backdrop-blur-sm px-3 py-1 rounded-full font-bold flex items-center space-x-1">
              <Sparkles className="w-3 h-3" />
              <span className="text-sm">Question {questionNumber} of {totalQuestions}</span>
            </div>
          ...
          <div className="flex items-center space-x-3 text-white/90">
            ...
            <div className={`flex items-center space-x-1 backdrop-blur-sm px-3 py-1 rounded-full font-bold ${getTimerColor()} text-black ${questionTimer <= 10 ? 'animate-pulse' : ''}`}>
              <AlertTriangle className="w-3 h-3" />
              <span className="font-mono text-sm">{questionTimer}s</span>
            </div>
            ...
            <div className="flex items-center space-x-1 bg-white/20 backdrop-blur-sm px-2 py-1 rounded-full">
              ...
              <span className="font-semibold text-xs">{question.points} pts</span>
            </div>
          </div>
        </div>
      </div>
        ...
        {questionTimer <= 10 && !isSubmitted && (
          <div className="bg-red-50 border-2 border-red-200 rounded-xl p-3 mb-3">
            <div className="flex items-center space-x-2 text-red-700">
              ...
              <span className="font-bold text-sm">Time Warning: Only {questionTimer} seconds remaining!</span>
            </div>
          </div>
        )}
      </div>
        ...
        <div className="space-y-4">
          <div className="relative">
            <Textarea
              ...
              className="min-h-[140px] text-base border-2 border-purple-200 focus:border-purple-500 transition-all duration-300 rounded-2xl p-4 bg-gradient-to-br from-purple-50/50 to-pink-50/50 resize-none"
          ...
          <div className="flex justify-between items-center">
            ...
            <Button
              ...
              className="bg-gradient-to-r from-purple-500 via-pink-500 to-blue-500 hover:from-purple-600 hover:via-pink-600 hover:to-blue-600 text-white px-6 py-3 rounded-2xl font-bold disabled:opacity-50 shadow-lg transform hover:scale-105 transition-all duration-200 flex items-center space-x-2 text-sm"
            >
